        ...
        <div class="container">
            <div class="account-wrapper">
                ...
                @endif
                    ...
                    <form class="account-form" method="post" action="{{ route('winner.store') }}">
                        @csrf
                        <div class="form-group">
                            <select required name="match_id" aria-label="SELECT MATCH">
                                <option selected disabled>SELECT MATCH</option>
                                @foreach($matches as $match)
                                    <option value="{{ $match->id }}">{{ $match->team_a }} V {{ $match->team_b }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <input oninput="this.value = this.value.toUpperCase();" required type="text" placeholder="Winner Club" id="winner_club" name="winner_club">
                        </div>
                        <div class="form-group">
